 ...
 * the count is retried with heuristically attributed hits passable again — the mis-attributed
 * cells of the still-live ship are among them. Sinking shots stay solid: the engine only
 * reports `sunk` on the shot that completed that ship. Only if that also finds nothing does
 * the map fall back to the plain hunt count.
 */
export const densityMap = (view: AIView): Density => {
  const mem = replay(view.shots);
  const certain = new Set<string>();
  for (const s of view.shots) if (s.result !== 'hit') certain.add(coordKey(s.coord));
  const unresolved = new Set(mem.unresolved.map((h) => coordKey(h.coord)));
  const blocked = new Set(certain);
  ...
  if (unresolved.size > 0) {
    ...
    const recovered = count({ ...k, blocked: certain }, true);
